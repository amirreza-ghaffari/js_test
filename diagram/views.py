...
def block_info_view(request, pk):
    ...
    context['receivers'] = block.members.all()

    try:
        history_approve = block.history.filter(history_date__gte=triggered_date,
                                               is_approved=True).order_by('-history_date').values('history_date',
